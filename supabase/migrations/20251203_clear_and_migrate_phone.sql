-- Migration: Clear all data and set up phone-based customers
-- This will DELETE all existing bookings and customers data
...
-- Step 3: Make phone NOT NULL since it's now the primary identifier
ALTER TABLE public.customers
ALTER COLUMN phone SET NOT NULL;

-- Step 4: Add new unique constraint on merchant_id + phone
ALTER TABLE public.customers
ADD CONSTRAINT customers_merchant_phone_unique UNIQUE (merchant_id, phone);

-- Step 5: Drop old indexes related to email uniqueness
DROP INDEX IF EXISTS idx_customers_email;
DROP INDEX IF EXISTS idx_customers_merchant_email;

-- Step 6: Create new indexes for phone
CREATE INDEX IF NOT EXISTS idx_customers_phone ON public.customers(phone);
CREATE INDEX IF NOT EXISTS idx_customers_merchant_phone ON public.customers(merchant_id, phone);

-- Step 7: Update the sync function to use phone instead of email
...
-- Note: No data to sync since we just cleared everything
-- New bookings will automatically create customers via the sync function
